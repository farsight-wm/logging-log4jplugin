package farsight.logging.log4j2;
 ...
 *      specification</a>
 */
@Plugin(name = "ISGelfLayout", category = Node.CATEGORY, elementType = Layout.ELEMENT_TYPE, printObject = true)
public class ISGelfLayout extends AbstractStringLayout {
	
	public static ISGelfLayout lastInstance;
	...
	public static class Builder<B extends Builder<B>> extends AbstractStringLayout.Builder<B>
			implements org.apache.logging.log4j.core.util.Builder<ISGelfLayout> {
		...
		@PluginBuilderAttribute
		private CompressionType compressionType = CompressionType.OFF;
		...
		@PluginBuilderAttribute
		private boolean includeNullDelimiter = true;
		...
		@Override
		public ISGelfLayout build() {
			return new ISGelfLayout(getConfiguration(), host, additionalFields, compressionType, compressionThreshold,
		...
		/**
		 * Whether to include full stacktrace of logged Throwables (optional,
		 * default to true). If set to false, only the class name and message of
		 * the Throwable will be included.
		 *
		 * @return this builder
		 */
		...
		/**
		 * Whether to include NULL byte as delimiter after each event (optional,
		 * default to false). Useful for Graylog GELF TCP input.
		 *
		 * @return this builder
		 */
	...
	/**
	     ...
	     */
	    @Deprecated
	    public ISGelfLayout(final String host, final KeyValuePair[] additionalFields, final CompressionType compressionType,
	...
	private ISGelfLayout(final Configuration config, final String host, final KeyValuePair[] additionalFields, final CompressionType compressionType,
	 ...
	 */
	@Deprecated
	public static ISGelfLayout createLayout(
			// @formatter:off
			@PluginAttribute("host") final String host,
			@PluginElement("AdditionalField") final KeyValuePair[] additionalFields,
			@PluginAttribute(value = "compressionType", defaultString = "OFF") final CompressionType compressionType,
			@PluginAttribute(value = "compressionThreshold", defaultInt = COMPRESSION_THRESHOLD) final int compressionThreshold,
			@PluginAttribute(value = "includeStacktrace", defaultBoolean = true) final boolean includeStacktrace) {
		// @formatter:on
		return new ISGelfLayout(null, host, additionalFields, compressionType, compressionThreshold, includeStacktrace,
				true, true);
	}
	...
	private StringBuilder toText(final LogEvent event, final StringBuilder builder, final boolean gcFree) {
		...
		if (additionalFields.length > 0) {
			...
			for (final KeyValuePair additionalField : additionalFields) {
				...
				final String value = valueNeedsLookup(additionalField.getValue())
						? strSubstitutor.replace(event, additionalField.getValue()) : additionalField.getValue();
		...
		if(message instanceof ISMessage) {
			//ESB Message
			final ISMessage esbMsg = (ISMessage) message;
			esbMsg.forEach(WRITE_KEY_VALUES_INTO, builder);
			
			builder.append("\"short_message\":\"");
			JsonUtils.quoteAsString(toNullSafeString(esbMsg.getText()), builder);
			builder.append(Q);
	...
	static CharSequence formatThrowable(final Throwable throwable) {
		// stack traces are big enough to provide a reasonably large initial
		// capacity here
